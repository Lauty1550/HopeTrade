...
function ExchangeListPage() {
  ...
  const fetchIntercambios = async () => {
    try {
      ...
      data.sort(function (a, b) {
        return (
          new Date(b.oferta.fechaIntercambio) -
          new Date(a.oferta.fechaIntercambio)
        );
      });
      if (verHoy) {
        ...
        data = data.filter(function (intercambio) {
          return (
            new Date(intercambio.oferta.fechaIntercambio).getDay() ==
            new Date().getDay()
          );
        });
      } else {
  ...
  const cancelar = async (id, motivo) => {
    let url = `/intercambio/cancelar/${id}`;
  ...
  const confirmar = async (id) => {
    let url = "/intercambio/confirmar/" + id;
    await axios
      .put(url)
      .then(function (response) {
        alert(response.data);
  ...
  return (
    <React.Fragment>
      <FormControl>
        <Select value={verHoy} onChange={handleSelect} sx={{ minWidth: 250 }}>
          <MenuItem value={false}>Todos</MenuItem>
          <MenuItem value={true}>Hoy</MenuItem>
        </Select>
      </FormControl>
      <PostGrid>
        {intercambios?.map((intercambio) => (
          <IntercambioItem
            id={intercambio.id}
            data={intercambio}
            publicacion={intercambio.publicacion}
            oferta={intercambio.oferta}
            user={user}
            cancelar={cancelar}
            confirmar={confirmar}
          />
        ...
        {!hayIntercambios ? (
          ...
          >
            <Typography variant="h1">
              No se encontraron intercambios programados para el dia de la
              fecha.
            </Typography>
          </div>
        ) : null}
