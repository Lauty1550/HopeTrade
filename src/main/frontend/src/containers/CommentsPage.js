...
function CommentsPage() {
  ...
  useEffect(() => {
    setDisableComentar(
      comentarioText.trim() === "" || comentarioText.length > 250
    );
  }, [comentarioText]);

  useEffect(() => {
    setDisableEnviarRespuesta(
      respuestaText.trim() === "" || respuestaText.length > 250
    );
  }, [respuestaText]);
  ...
  const fetchComentarios = async () => {
    try {
      ...
      const comentariosActivos =
        await ComentariosYRespuestasService.fetchComentarios(id);
      comentariosActivos.sort(
        (a, b) => new Date(b.fechaComentario) - new Date(a.fechaComentario)
      );
  ...
  const fetchRespuestas = async (comentarioId) => {
    try {
      const respuesta = await ComentariosYRespuestasService.fetchRespuestas(
        comentarioId
      );
  ...
  const guardarComentario = async () => {
    ...
    try {
      ...
      const response = await ComentariosYRespuestasService.guardarComentario(
        comentarioData
      );
      alert(response);
      setComentarioText("");
  ...
  const guardarRespuesta = async (comentarioId) => {
    ...
    try {
      ...
      const response = await ComentariosYRespuestasService.guardarRespuesta(
        respuestaData
      );
      alert(response);
      setRespuestaText("");
  ...
  const eliminarComentario = async (comentarioId) => {
    try {
      const response = await ComentariosYRespuestasService.eliminarComentario(
        comentarioId
      );
      alert(response);
      fetchComentarios();
            ...
            <Grid item xs={12} key={comentario.idComentario}>
              ...
              >
                ...
                {respuestas[comentario.idComentario] && (
                  ...
                  >
                    <Typography variant="body2">
                      {publicacion.userFullName} -{" "}
                      {respuestas[comentario.idComentario].fechaRespuesta &&
                        new Date(
                          respuestas[comentario.idComentario].fechaRespuesta
                        ).toLocaleString()}
                    </Typography>
                    <Typography variant="subtitle2">
                      {respuestas[comentario.idComentario].text}
                    </Typography>
                  </Box>
                )}
                {/* Formulario para responder */}
                {comentarioSeleccionado &&
                  comentarioSeleccionado.idComentario ===
                  ...
                  !respuestas[comentario.idComentario] && (
                    <Box sx={{ marginLeft: 2, marginTop: 1 }}>
                      ...
                      <Button
                        ...
                        color="primary"
                        onClick={() =>
                          guardarRespuesta(comentario.idComentario)
                        }
