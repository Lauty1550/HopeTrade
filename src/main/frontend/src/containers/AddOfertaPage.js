...
function AddOfertaPage() {
  ...
  const fetchFiliales = async () => {
    try {
      const response = await axios.get("/filial/all");
    ...
    switch (e.target.name) {
      ...
      case "imagen":
        ...
        if (file && !validImageTypes.includes(file.type)) {
          fileError =
            "Error al subir el archivo. Por favor use una imagen que posea los siguientes formatos: .jpg, .png, .bmp, .tif, or .webp";
          ...
          <Stack spacing={2} direction="column">
            <FormControl>
              <TextField
                ...
                name="titulo"
              />
              <FormHelperText id="titulo-text">
                Ingrese el título de su oferta
              </FormHelperText>
            </FormControl>
            <FormControl>
              <TextField
                ...
                variant="outlined"
              />
              <FormHelperText id="descripcion-text">
                Describa el producto ofertado
              </FormHelperText>
            </FormControl>
            <FormControl>
              <TextField
                value={post.categoria_Nombre}
                type="text"
                variant="outlined"
                id="apellido"
              ...
              />
              <FormHelperText id="categoria-text">
                {" "}
                Verifique que la oferta sea de la misma categoría que la
        ...
        <Grid item xs={4}>
          <Stack spacing={2} direction="column">
            <FormControl>
              ...
              <FormHelperText id="imagen-text">
                Agregue una foto del producto ofertado
              </FormHelperText>
            </FormControl>
            <FormControl>
              <Select
              ...
              >
                ...
                {filiales.map((filial) => (
                  <MenuItem value={filial.id}>
                    {filial.nombre} ({filial.direccion})
                  </MenuItem>
                ))}
              </Select>
              <FormHelperText id="filial-text">
                Seleccione la filial donde desea realizar el intercambio
              </FormHelperText>
            </FormControl>
            <FormControl>
              <DateTimePicker
                onChange={(event) => {
                  handleChange({ ...event, target: { name: "fechaHora" } });
                }}
                variant="outlined"
                name="fechaHora"
                defaultValue={defDateTime}
                minDate={nextDay}
                maxDate={nextMonth}
              ...
              />
              <FormHelperText id="fechaHora-text">
                Indique fecha y hora deseadas para realizar el intercambio
              </FormHelperText>
            </FormControl>
            <Button
