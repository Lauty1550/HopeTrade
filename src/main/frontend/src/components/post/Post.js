...
function Post(props) {
  ...
  const deletePost = async () => {
    ...
    try {
      await axios.put("/publicacion/eliminar/" + id);
  ...
  const adminDeletePost = async () => {
    console.log("Eliminando publicación con ID:", id);
    const url = `/publicacion/eliminar/${id}`;
        ...
        <Grid container alignItems="center" spacing={1}>
          ...
          <Grid item xs={4}>
            <Avatar
              src={data.imagenUrl}
              variant="rounded"
              sx={{ marginLeft: "15px", width: "150px", height: "150px" }}
            />
          </Grid>
          <Grid item xs={12}>
            <Stack spacing={2} direction="row">
              ...
              {((user.tipoUser > 0 && !isCommentsPage) ||
                (user.idUser === data.userID &&
                  data.estado === "Disponible" &&
                  !isCommentsPage &&
                  user.tipoUser === 0)) && (
              ...
              {user.idUser === data.userID &&
                data.estado === "Disponible" &&
                isCommentsPage &&
                !window.location.href.includes("/inspect-post") &&
                data.ofertas > 0 && (
