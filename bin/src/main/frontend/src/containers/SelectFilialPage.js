...
function SelectFilialPage() {
  ...
  const handleSelect = async () => {
    if (userId && selectedFilial) {
      try {
        const response = await FilialService.selectFilial(
          userId,
          selectedFilial
        );
        ...
        if (cookie) {
          ...
          let laFilial = filiales.find(
            (element) => element.id === selectedFilial
          );
          user = {
            ...user,
            filial: selectedFilial,
            desc_filial: laFilial.direccion + " (" + laFilial.nombre + ")",
          };
